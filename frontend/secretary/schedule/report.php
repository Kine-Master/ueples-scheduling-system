...
$type = trim($_GET['schedule_type'] ?? '');
$semester = trim($_GET['semester'] ?? '');
$day = trim($_GET['day_of_week'] ?? '');
...
if ($teacherId) {
    ...
    if ($teacherRow) {
        $teacherLabel = trim(($teacherRow['last_name'] ?? '') . ', ' . ($teacherRow['first_name'] ?? ''));
    }
}
...
$semesterLabel = $semester === '1' ? '1st Semester' : ($semester === '2' ? '2nd Semester' : ($semester === 'Summer' ? 'Summer' : 'Any Semester'));
$filters = [];
$filters[] = 'Type: ' . ($type ?: 'All');
$filters[] = 'Semester: ' . $semesterLabel;
$filters[] = 'Day: ' . ($day ?: 'All Days');
$filters[] = 'Teacher: ' . ($teacherLabel ?: 'All Teachers');
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Master Schedule Report</title>
  <style>
...
<body onload="window.print()">
  ...
  <div class="header">
    <h1>UEP LES Scheduling System - Master Schedule Report</h1>
    <div class="meta"><strong>School Year:</strong> <?= e($schoolYearLabel) ?></div>
    <div class="meta"><strong>Filters:</strong> <?= e(implode(' | ', $filters)) ?></div>
  ...
  <div class="sig-wrap">
    ...
    <div class="sig"><div class="line">Checked By</div><div>Principal</div></div>
    <div class="sig"><div class="line">Approved By</div><div>Administrator</div></div>
  </div>
...
function buildBlocks(rows) {
  ...
  rows.forEach(r => {
    ...
    const teacher = r.teacher_name || 'Teacher';

    blocks.set(key, {
      type: r.schedule_type,
      label: `<strong>${esc(subject)}</strong><br>${esc(teacher)}<br>${esc(section)}<br>${esc(room)}`,
      span
    });
